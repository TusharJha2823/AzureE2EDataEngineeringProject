...
-- MAGIC %md
-- MAGIC Within the legacy Hive metastore local to this workspace, we now have a table called **movies**, residing in a the user-specific database outlined in the cell output from above. To make things easier, the database name is stored in a Hive variable named *da.schema_name*. Let's preview the data stored in this table using that variable.
